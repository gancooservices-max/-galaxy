import { Body, MakeTime, HelioVector, GeoVector, Observer, SiderealTime } from 'astronomy-engine';
...
export const PLANETS = [
  ...
  { id: 'Moon', body: Body.Moon, texture: '/textures/moon.png', color: '#dddddd', size: 3.5, mag: -12.7, desc: "Earth's only natural satellite." },
...
export function getPlanetsData(date) {
  ...
  // Convert Ecliptic to Equatorial (tilt by 23.44 degrees)
  const e = 23.4392911 * (Math.PI / 180);
  const cosE = Math.cos(e);
  const sinE = Math.sin(e);
  
  const toEquatorial = (v) => {
    return {
      x: v.x,
      y: v.y * cosE - v.z * sinE,
      z: v.y * sinE + v.z * cosE
  ...
  const earthEq = toEquatorial(earthVec);
  const earthPos = new THREE.Vector3(earthEq.x, earthEq.z, -earthEq.y).multiplyScalar(AU_SCALE);
  ...
  return PLANETS.map(p => {
    ...
    if (p.id === 'Sun') {
      ...
      const moonEq = toEquatorial(geoVec);
      const moonRel = new THREE.Vector3(moonEq.x, moonEq.z, -moonEq.y).multiplyScalar(AU_SCALE);
      pos.copy(earthPos).add(moonRel);
    } else {
      const hVec = HelioVector(p.body, time);
      const eqVec = toEquatorial(hVec);
      pos.set(eqVec.x, eqVec.z, -eqVec.y).multiplyScalar(AU_SCALE);
    }
...
/**
 * Calculates the Greenwich Mean Sidereal Time (GMST) in hours.
 */
export function calculateLST(date) {
  const time = MakeTime(date);
